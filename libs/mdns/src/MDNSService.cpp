...
{
   public:
    ServiceThread(std::atomic<bool>& running, int socket, mdns_record_callback_fn callback, void* user_data)
        : running_(running), socket_(socket), callback_(callback), user_data_(user_data)
    {
    }
...
{
   public:
    MDNSServiceImpl() : running_(false), serviceSocket_(-1)
    {
