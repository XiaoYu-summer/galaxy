...
void PassportApiController::InitAdminRoutes(CrowApp& app)
{
    CROW_ROUTE(app, "/passport/api/v1/admin/token/generate")
        .methods("POST"_method)([&app](const crow::request& request, crow::response& response)
            {
                HandleJsonReqWithParams<std::string, std::string>(request, response, [&app, &request](const std::string& accountName, const std::string& password, crow::response& response)
                    {
    ...
    CROW_ROUTE(app, "/passport/api/v1/admin/token/change")
        .methods("PUT"_method)([&app](const crow::request& request, crow::response& response)
            {
                HandleJsonReqWithParams<std::string, std::string>(request, response, [&app, &request](const std::string& oldPassword, const std::string& newPassword, crow::response& response)
                    {
...
void PassportApiController::InitClientRoutes(CrowApp& app)
{
    CROW_ROUTE(app, "/passport/api/v1/client/pairing")
        .methods("POST"_method)([&app](const crow::request& request, crow::response& response)
            {
                HandleJsonReqWithParams<std::string>(request, response, [](const std::string& pairingCode, crow::response& response)
                    {
